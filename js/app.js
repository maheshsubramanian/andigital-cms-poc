    ...
    app.service('GlobalService', [function () {
        ...
        var parallax = function () {
            ...
            $('.parallax').each(function () {
                var pos = (Math.round(((scrolled) * 0.5)));
                $(this).velocity({translateY: -pos + 'px'}, {duration: 0});
            });
        };
    }]);
})();
...
(function () {
    ...
    app.controller('HeaderController', ['$scope', function ($scope) {
        ...
        $(window).scroll(function () {
            ...
            if (offset > 100) {
                setHeader('normal');
                $('#share-icon-cont').css("color", "#d93621");
            } else {
                setHeader('transparent');
                $('#share-icon-cont').css("color", "#FFFFFF");
            }
        });
...
(function () {
    ...
    app.controller('HeroImageController', ['$scope', function ($scope) {
        ...
        $(window).scroll(function () {
            ...
            if (offset > 600) {
                ...
                $('.hero-image').css("visibility", "visible");
            }
        });
...
(function () {
    ...
    app.controller('WhatWeDoController', ['$scope', 'GlobalService', function ($scope, GlobalService) {
        ...
        var setActiveService = function (index) {
            ...
            $('[data-service="' + activeService + '"]').hide();
            $('[data-service="' + index + '"]').show();
        ...
        //CLIENTS CAROUSEL
        var init = function () {
            $('[data-service="' + activeService + '"].services-row').show();
...
(function () {
    ...
    app.controller('WhoWeAreController', ['$scope', 'GlobalService', function ($scope, GlobalService) {
        ...
        $scope.productAnalyst = 'Product Analyst';
        $scope.productDeveloper = 'Product Developer';
        $scope.squadLead = 'Squad Lead';
        $scope.associate = 'Associate';

        //  Roles object contains the label for each role (applied while using the filter) and the description
        var roles = {};
        roles[$scope.productAnalyst] = {
            'label': 'Product Analysts',
            'blurb': 'Our bright spark Product Analysts work within Agile Scrum teams and are the experts in digital and e-commerce function and design. These analytical whizzes help our clients deliver outstanding products by developing clever technical solutions that best fit the business needs.'
        };
        roles[$scope.productDeveloper] = {
            'label': 'Product Developers',
            'blurb': 'Our tech savvy Developers work within Agile Scrum teams and are instrumental in helping build digital, online and eCommerce products with our clients. These front end, back end or full stack Developers work together on innovative and brilliant digital products that knock our clients socks off. '
        };
        roles[$scope.squadLead] = {
            'label': 'Squad Leads',
            'blurb': 'Our Squad Leads head up our different squads, guiding and coaching them to build and develop exceptional web based products for our clients. Our Squad Leads, are also experienced digital gurus who themselves, take on hands-on roles working closely with clients.'
        };
        roles[$scope.associate] = {
            'label': 'Associates',
            'blurb': 'What A do...'
        };

        //  The filter* functions are being used by the PHP page once the filter selection is triggered by the user
        var filterAnalysts = function () {
            applyJobTitleFilter($scope.productAnalyst);
        };

        var filterDevelopers = function () {
            applyJobTitleFilter($scope.productDeveloper);
        };

        var filterSquadLeads = function () {
            applyJobTitleFilter($scope.squadLead);
        };

        var filterAssociates = function () {
            applyJobTitleFilter($scope.associate);
        };

        //  The function filters and sorts people depending on the filter term in input
        //  If the filter is Analyst or Developer it also concatenates Associates to the resulting array
        //  The function also uses the $scope.filterCriterion as a toggle mutex to apply and unapply the filter
        var applyJobTitleFilter = function (filterTerm) {
            if ($scope.filterCriterion !== filterTerm) {
                //  Set filter
                $scope.filterCriterion = filterTerm;
                //  Filter data are used to display information about the current jobTitle (filterTerm)
                $scope.filterData = roles[filterTerm];
                var filteredArray = sortPeopleByFirstname(filterPeopleByJobTitle(filterTerm));
                //  If analyst or developer, add associates:
                if (filterTerm == $scope.productAnalyst || filterTerm == $scope.productDeveloper) {
                    var associates = sortPeopleByFirstname(filterPeopleByJobTitle($scope.associate));
                    //  Append associates:
                    filteredArray = filteredArray.concat(associates);
                }
                $scope.filteredArray = filteredArray;
            }
            else {
                //  Reset filter to its original value
                $scope.filterCriterion = '';
                $scope.filterData = {};
            }
        };

        //  The function simply gets the array of People and filters them by jobtitle (starting with, to include also
        //  "Associate Product Developer" with "Associate")
        var filterPeopleByJobTitle = function (jobTitle) {

            var filteredArray = people.filter(function (element, index, array) {
                if (element.jobTitle)
                    return ( (element.jobTitle).startsWith(jobTitle) );
                return false;
            });
        ...
        //  The function sorts the array of people alphabetically by their first name
        var sortPeopleByFirstname = function (array) {
        ...
        var buildProfile = function (index, array, memberIdWithinGroup, totalGroupMembers) {
            if (!array) {
                array = people;
            }
            ...
            if (!person.role) {
                profile.find('.role-lbl').html('');
            }
            if (person.hobbies) {
                profile.find('.super').html("Hobbies");
                profile.find('.profile-superhero-power').html(person.hobbies);
            } else {
                profile.find('.profile-superhero-power').html(person.superheroPower);
            }

            if (person.githubLink) {
              profile.find('.profile-github-link').attr('href', person.githubLink);
              profile.find('.profile-github-col').css('display', 'block');
            }
            if (person.blogLink) {
              profile.find('.profile-blog-link').attr("href", person.blogLink);
              profile.find('.profile-blog-col').css('display', 'block');
            }
            if (person.githubLink && person.blogLink) {
              profile.find('.profile-github-col').removeClass('col-sm-12').addClass('col-sm-6');
              profile.find('.profile-blog-col').removeClass('col-sm-12').addClass('col-sm-6');
            }
            ...
            //  Find out the ID of the last element of the row the current member (memberIdWithinGroup) belongs to
            var rowLastElement = Math.ceil(memberIdWithinGroup / getColumns()) * getColumns();
            if (rowLastElement > totalGroupMembers) {
                rowLastElement = totalGroupMembers;
            }

            var persons = $('.person');
            var position = (index - memberIdWithinGroup + 1) + (rowLastElement - 1);

            $(persons[position]).after(profile.show());
            profile.velocity("scroll", {duration: 600, offset: -73});
        };
        ...
        var selectPerson = function (index, array, memberIdWithinGroup, totalGroupMembers) {
            $('.profile').remove();
            currentProfile = index;
            buildProfile(index, array, memberIdWithinGroup, totalGroupMembers);
        };
        ...
        $scope.filterAnalysts = filterAnalysts;
        $scope.filterDevelopers = filterDevelopers;
        $scope.filterSquadLeads = filterSquadLeads;

    }]);
})();


// Begin controllers specific to ANDacademy page
...
(function () {
    ...
    app.controller('CarouselController', ['$scope', function ($scope) {
        ...
        var init = function () {
            $("#clients-quotes-carousel").owlCarousel({
                loop: false,
                center: true,
                items: 1,
                margin: 100,
...
(function () {
    ...
    app.controller('InPageNavController', ['$scope', function ($scope) {
...
// End controllers specific to ANDacademy page
    ...
    app.directive('scroll', ['$window', function($window) {
        return {
            ...
            link: function (scope, element, attrs) {
                scope.scrollHere = function($event) {
                    if($(window).width() < 993 && $event.currentTarget.className.indexOf('collapsed')>-1){
                        
                        setTimeout(function() {
                            $('html, body').animate({
                                scrollTop: element.find('.in').offset().top - 126
                            }, 600);
                        }, 500);
                    }
                }
